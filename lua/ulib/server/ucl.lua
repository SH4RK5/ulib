...
function ucl.addGroup(name, allows, inherit_from, from_CAMI)
	...
	if inherit_from then
		...
		if not ucl.groups[inherit_from] then return error(
			"Invalid group for inheritance (" .. tostring(inherit_from) .. ")", 2) end
	end
...
function ucl.setGroupInheritance(group, inherit_from, from_CAMI)
	...
	if inherit_from and not ucl.groups[inherit_from] then return error(
		"Group for inheritance does not exist (" .. inherit_from .. ")", 2) end
	...
	while groupCheck do
		if groupCheck == group then               -- Got back to ourselves. This is bad.
			...
			error(
			"Changing group \"" ..
			group .. "\" inheritance to \"" .. inherit_from .. "\" would cause cyclical inheritance. Aborting.", 2)
		end
